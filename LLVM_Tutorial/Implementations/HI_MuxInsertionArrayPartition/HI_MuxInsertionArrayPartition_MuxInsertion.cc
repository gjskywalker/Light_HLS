...
{
    if (DEBUG)
        *BRAM_log << "---- Evaluating MuxInsert for Block: " << B->getName() << ":\n";
            ...
            for (Instruction *I : LoadInsts)
            {
                ...
                if (I->getOpcode() == Instruction::Load /* || I->getOpcode()==Instruction::Store */)
                {
                    ...
                    if (target_partitions.size() > 1)
                    {
                        ...
                        {
                            muxWithMoreThan32 = 1;
                            std::string tmp_loop_name = LI->getLoopFor(B)->getHeader()->getParent()->getName().str();
                            tmp_loop_name += "-";
                        ...
                        if (DEBUG)
                            *BRAM_log << "----------- A Memory Access Instruction: " << *I
                        ...
                        std::string funcName = "HIPartitionMux" + std::to_string(target_partitions.size()) + "_" +
                                               std::to_string(mux_cnt++);
                        Value *newF_val = B->getParent()
                                              ->getParent()
                                              ->getOrInsertFunction(funcName, FunctionType::get(RetTy, ArgTys, false))
                                              .getCallee();
                        ...
                        ArgsForMux.push_back(
                            ConstantInt::get(llvm::Type::getInt32Ty(B->getContext()), target_partitions.size()));

                        Value *newMux = Builder.CreateCall(newF->getFunctionType(), newF_val, ArgsForMux);
                        if (DEBUG)
    ...
    while (replaced)
    {
        ...
        for (auto tmp_use = Old->use_begin(), tmp_use_end = Old->use_end(); tmp_use != tmp_use_end; tmp_use++)
        {
            ...
            if (U.getUser() != New)
            {
                ...
                U.set(New);
                if (DEBUG)
                    *BRAM_log << "----------- replaced use : [" << *U.getUser() << "]\n";
                break;
            }
            if (DEBUG)
                *BRAM_log << "\n";

        ...
        for (Instruction &I : B)
        {
            if (I.getOpcode() == Instruction::Store)
            {
                if (I.getOperand(0)->getType()->isIntegerTy())
                {
                    ...
                    // assert(PtrType);
                    // Instruction_BitNeeded[&I] = PtrType->getPointerElementType()->getIntegerBitWidth();
                    Instruction_BitNeeded[&I] = SI->getValueOperand()->getType()->getIntegerBitWidth();
                }
            }
...
unsigned int HI_VarWidthReduce::bitNeededFor(ConstantRange CR)
{
    ...
    else
    {
        ...
        unsigned int lowerNeedBits = CR.getMinSignedBits();
        unsigned int upperNeedBits = CR.getMinSignedBits();

                 ...
                 // will be executed for each Module.
{
    ...
    while (1)
    {
        ...
        if (DT.getRoot() == cur_block) // The entry block is the root of the Dominate Tree, and the traverse is from the bottum to the top
                                       // so when traverse arrived the entry block, the traverse is ended.
            break;
    ...
    //             if (auto PHI_I = dyn_cast<PHINode>(&I))
    //             {
    //                 int num_op = PHI_I->getNumOperands();
    //                 for (int i=0;i<num_op;i++)
    //                 {
    //                     if (auto tmp_I = dyn_cast<Instruction>(PHI_I->getOperand(i)))
    //                     {
    //                         if (tmp_I->getParent() == &B)
    //                         {
    //                             if (Instruction_Visited.find(&I) != Instruction_Visited.end())
    //                             {
    //                                 I.moveBefore(PHI_I->getIncomingBlock(i)->getTerminator());
    //                                 modified = true;
    //                                 break;
    //                             }
    //                         }

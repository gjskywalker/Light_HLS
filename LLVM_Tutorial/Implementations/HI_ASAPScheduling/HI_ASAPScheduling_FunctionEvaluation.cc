...
*/
HI_ASAPScheduling::timingBase HI_ASAPScheduling::analyzeFunction(Function *F)
{
    ...
    *Evaluating_log << "-- its entry is: " << Func_Entry->getName() << "\n";
    ...
    // (2) tracerse the block in loop by DFS to find the longest path
    timingBase max_critical_path_in_F(0, 0, 1, clock_period);
    ...
    for (auto tmp_it : tmp_BlockCriticalPath_inFunc)
        if (tmp_it.second > max_critical_path_in_F)
            max_critical_path_in_F = tmp_it.second;
    ...
    // loop as in integration
    Func_BlockVisited.clear();
    FunctionLatency[F] = max_critical_path_in_F;
    FunctionEvaluated.insert(F);

    *Evaluating_log << "\n\n Done evaluation Function Latency for Function " << F->getName()
                    << " and its latency is " << max_critical_path_in_F << ".\n";
    ...
    *Evaluating_log << "\n\n\n\n";
    return max_critical_path_in_F * 1;
}
...
/*
    ...
    (3a) -- If it is a block in loops, regard the most inner outer loop as intergration and update the
    critical path if necessary (max(ori_cP, lasrStateCP + LoopLatency)).
         -- Find the successors of the loop by checking its exiting blocks' successors and cotinue the DFS
...
*/
void HI_ASAPScheduling::analyzeFunction_traverseFromEntryToExiting(timingBase tmp_critical_path, Function *F, BasicBlock *curBlock)
{
...
int HI_ASAPScheduling::getFunctionStageNum(
    HI_ASAPScheduling::timingBase tmp_critical_path, Function *F,
    BasicBlock *curBlock)
{

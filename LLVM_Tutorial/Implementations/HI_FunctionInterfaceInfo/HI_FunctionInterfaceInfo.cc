...
void HI_FunctionInterfaceInfo_Visitor::printTypeInfo(const clang::Type *T)
{
...
bool HI_FunctionInterfaceInfo_Visitor::isAPInt(VarDecl *VD)
{
...
std::string HI_FunctionInterfaceInfo_Visitor::getAPIntName(VarDecl *VD)
{
    const Type *T = VD->getType().getTypePtr();
    if (const TemplateSpecializationType *TST = dyn_cast<const TemplateSpecializationType>(T))
    {

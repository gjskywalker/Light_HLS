                    ...
                    if (isa<ConstantArrayType>(arg->getOriginalType()))
                    {
                        ...
                        *parseLog << "outermost dimensionSize = " << arraySize.getSExtValue();
                        FuncParamLine2OutermostSize[FuncName + "-" + arg->getNameAsString() + "-" +
                                                    std::to_string(FSL.getLineNumber())] = arraySize.getSExtValue();
                        *parseLog << "recorded as : "
                                  << FuncName + "-" + arg->getNameAsString() + "-" + std::to_string(FSL.getLineNumber())
...
class HI_FunctionInterfaceInfo_ASTConsumer : public ASTConsumer
{
  public:
    HI_FunctionInterfaceInfo_ASTConsumer(CompilerInstance &_CI, Rewriter &R, std::string _parselog_name,
                                         std::map<std::string, int> &FuncParamLine2OutermostSize,
                                         std::string topFunctioName, bool functionAllInline)
        : Visitor(_CI, R, _parselog_name, FuncParamLine2OutermostSize, topFunctioName, functionAllInline), CI(_CI),
          parselog_name(_parselog_name), topFunctioName(topFunctioName), functionAllInline(functionAllInline)
    {
...
template <typename T>
std::unique_ptr<tooling::FrontendActionFactory> HI_FunctionInterfaceInfo_rewrite_newFrontendActionFactory(
    ...
    std::map<std::string, int> &FuncParamLine2OutermostSize, std::string topFunctioName, bool functionAllInline = 0)
{
    class SimpleFrontendActionFactory : public tooling::FrontendActionFactory
    {
      public:
        ...
        {
            std::unique_ptr<FrontendAction> frontendaction(new T(parseLog_name.c_str(), TheRewriter,
                                                                 outputCode_name.c_str(), FuncParamLine2OutermostSize,
                                                                 topFunctioName, functionAllInline));
    ...
    return std::unique_ptr<tooling::FrontendActionFactory>(new SimpleFrontendActionFactory(
        _parseLog_name, R, _outputCode_name, FuncParamLine2OutermostSize, topFunctioName, functionAllInline));
}

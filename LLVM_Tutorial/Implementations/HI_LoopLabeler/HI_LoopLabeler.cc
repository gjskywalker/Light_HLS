...
void HI_LoopLabeler_Visitor::printTypeInfo(const clang::Type *T)
{
...
bool HI_LoopLabeler_Visitor::isAPInt(VarDecl *VD)
{
...
std::string HI_LoopLabeler_Visitor::getAPIntName(VarDecl *VD)
{
    const Type *T = VD->getType().getTypePtr();
    if (const TemplateSpecializationType *TST = dyn_cast<const TemplateSpecializationType>(T))
    {

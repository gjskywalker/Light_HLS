...
public:
    ...
    {
        *parseLog << "find VarDecl: VarName: [" << VD->getNameAsString() << "] DeclKind:[" << VD->getDeclKindName()
                  << "] Type: [" << VD->getType().getAsString() << "] at Loc: ["
        ...
        *parseLog << "    ---  detailed information of the type\n";
        ...
        if (isAPInt(VD))
        {
            TheRewriter.InsertText(VD->getBeginLoc(),
                                   "// " + VD->getNameAsString() + " is ap int type (" + getAPIntName(VD) + ").\n");
        }
        parseLog->flush();
        return true;
    }
...
HI_LoopLabeler_rewrite_newFrontendActionFactory(const char *_parseLog_name, Rewriter &R, const char *_outputCode_name)
{
    ...
    public:
        ...
        {
            std::unique_ptr<FrontendAction> frontendaction(
                new T(parseLog_name.c_str(), TheRewriter, outputCode_name.c_str()));

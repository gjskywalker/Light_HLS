...
HI_rewrite_newFrontendActionFactory(const char *_parseLog_name, Rewriter &R, const char *_outputCode_name)
{
    class SimpleFrontendActionFactory : public tooling::FrontendActionFactory
    {
      public:
        ...
        {
            return std::make_unique<T>(parseLog_name.c_str(), TheRewriter, outputCode_name.c_str());
        }
        std::string parseLog_name;
        std::string outputCode_name;
        Rewriter &TheRewriter;
